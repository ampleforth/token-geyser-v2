...
export const StakeUnstakeView: React.FC= () => {
  ...
  const { selectedVault: vault, geysers } = useContext(VaultsContext)
  const [mode, setMode] = useState<Mode>('Stake')
  const [amount, setAmount] = useState<string>('')
  const [selectedGeyser, setSelectedGeyser] = useState<string>(geysers.length > 0 ? geysers[0].id : '')
  ...
  return (
    <>
      {vault && vault.locks.length > 0
        ? <StakesContainer>
            {vault?.locks.map(lock => <StakeListItem lock={lock} />)}
          </StakesContainer>
